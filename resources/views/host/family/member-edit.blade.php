		...
		<div class="container">
			<div class="row">
				<div class="col-sm-8">
					...
					<form class="form-group" method="POST" action="{{ route('family.update', $member->id) }}">
			        	...
			        	<div class="form-group row{{ $errors->has('first_name') ? ' has-error' : '' }}">
				            ...
				            <div class="col col-md-8">
				            	<input id="first_name" type="text" class="form-control" name="first_name" placeholder="First Name" value="{{ old('first_name', $member->first_name) }}" required autofocus>
				            </div>
				        ...
				        <div class="form-group row{{ $errors->has('last_name') ? ' has-error' : '' }}">
				            ...
				            <div class="col col-md-8">
				            	<input id="last_name" type="text" class="form-control" name="last_name" placeholder="Family Name" value="{{ old('last_name', $member->last_name) }}" required>
				            </div>
				        ...
				        <div class="form-group row{{ $errors->has('date_birth') ? ' has-error' : '' }}">
				            ...
				            <div class="col col-md-8">
				            	<input id="date_birth" type="date" class="form-control" name="date_birth" placeholder="Date of Birth" value="{{ old('date_birth', $member->date_birth) }}" required>
				            </div>
    						...
    						<div class="col col-md-8">
	    						<select class="form-control" id="relationship" name="relationship">
								     @foreach ($options as $key => $value)
				            			<option value="{{ $key }}"
				            			@if ($key == old("relationship", $member->relationship))
				            				selected="selected"
				            			@endif
				            			> {{ $value }}</option>
				            		@endforeach
